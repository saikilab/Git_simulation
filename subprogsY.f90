...
subroutine interaction(total_particle_num,i,j,distances,&
  mag_permeability,mag_force,mu1_v,mu2_v)
  implicit none
  integer,intent(in)::total_particle_num,i,j
  double precision,intent(in)::mag_permeability,mu1_v(3),mu2_v(3)
  double precision,intent(in)::distances(total_particle_num,total_particle_num,4)
  double precision,intent(out) :: mag_force(3,total_particle_num)
  double precision :: r, dmf(3) !delta_mag_force_x,y,z, delta_H_force_x,y,z
  double precision, parameter :: pi = dacos(-1.0d0)

  r = sqrt(distances(i,j,4))

  !force between dipoles
  dmf(1) = 3 * mag_permeability / (4 * pi * r**4) *( &
  dot_product( mu1_v(:), distances(i,j,1:3) / r) * mu2_v(1) &
  + dot_product(mu2_v(:), distances(i,j,1:3) / r) * mu1_v(1) &
  + dot_product(mu1_v(:), mu2_v(:)) * distances(i,j,1) / r &
  - 5 * dot_product(mu1_v(:), distances(i,j,1:3) / r) * dot_product(mu2_v(:), distances(i,j,1:3) / r) * distances(i,j,1) / r)

  dmf(2) = 3 * mag_permeability / (4 * pi * r**4) * ( dot_product( &
  mu1_v(:), distances(i,j,1:3) / r) * mu2_v(2) &
  + dot_product(mu2_v(:), distances(i,j,1:3) / r) * mu1_v(2)&
  + dot_product(mu1_v(:), mu2_v(:)) * distances(i,j,2) / r - 5 * &
  dot_product(mu1_v(:), distances(i,j,1:3) / r) * dot_product(mu2_v(:), distances(i,j,1:3) / r) * distances(i,j,2) / r)

  dmf(3) = 3 * mag_permeability / (4 * pi * r**4) * (dot_product(&
  mu1_v(:), distances(i,j,1:3) / r) * mu2_v(3)&
  + dot_product(mu2_v(:), distances(i,j,1:3) / r) * mu1_v(3)&
  + dot_product(mu1_v(:), mu2_v(:)) * distances(i,j,3) / r - 5 * &
  dot_product(mu1_v(:), distances(i,j,1:3) / r) * dot_product(mu2_v(:), distances(i,j,1:3) / r) * distances(i,j,3) / r)

  mag_force(1,i) = mag_force(1,i) + dmf(1)
  mag_force(1,j) = mag_force(1,j) - dmf(1)
  mag_force(2,i) = mag_force(2,i) + dmf(2)
  mag_force(2,j) = mag_force(2,j) - dmf(2)
  mag_force(3,i) = mag_force(3,i) + dmf(3)
  mag_force(3,j) = mag_force(3,j) - dmf(3)

end subroutine interaction


subroutine  ext_mag_field(total_particle_num,mu1,r_H,r_H_v,H,theta_mu,&
  phi_mu,mag_permeability,mag_force)
  implicit none
  integer,intent(in)::total_particle_num
  double precision,intent(in)::mu1,theta_mu,phi_mu,r_H_v(3)
  double precision,intent(in)::mag_permeability,r_H,H
  double precision,intent(inout) :: mag_force(3,total_particle_num)
  double precision :: mu_v(3),H_v(3)! _v:vector
  double precision :: dHf(3) !delta_mag_force_x,y,z, delta_H_force_x,y,z
  double precision, parameter :: pi = dacos(-1.0d0)

  mu_v = 0

  mu_v(1) = mu1 * sin(theta_mu) * cos(phi_mu)
  mu_v(2) = mu1 * sin(theta_mu) * sin(phi_mu)
  mu_v(3) = mu1 * cos(theta_mu)
  H_v = r_H_v * H
  !H_v = 0

  !force between dipole amd ,external magnetic field
  dHf(1) =3 * mag_permeability / (4 * pi * r_H**4) *( dot_product( &
  mu_v(:),r_H_v) * H_v(1) + dot_product(H_v, r_H_v) &
  * mu_v(1) + dot_product(mu_v(:), H_v) * r_H_v(1) - 5 * &
  dot_product(mu_v(:), r_H_v) * dot_product(H_v, r_H_v) * &
  r_H_v(1))

  dHf(2) =3 * mag_permeability / (4 * pi * r_H**4) *( dot_product( &
  mu_v(:),r_H_v) * H_v(2) + dot_product(H_v, r_H_v) &
  * mu_v(2) + dot_product(mu_v(:), H_v) * r_H_v(2) - 5 * &
  dot_product(mu_v(:), r_H_v) * dot_product(H_v, r_H_v) * &
  r_H_v(2))

  dHf(3) =3 * mag_permeability / (4 * pi * r_H**4) *( dot_product( &
  mu_v(:),r_H_v) * H_v(3) + dot_product(H_v, r_H_v) &
  * mu_v(3) + dot_product(mu_v(:), H_v) * r_H_v(3) - 5 * &
  dot_product(mu_v(:), r_H_v) * dot_product(H_v, r_H_v) * &
  r_H_v(3))


  mag_force(1,:) = mag_force(1,:) + dHf(1)
  mag_force(2,:) = mag_force(2,:) + dHf(2)
  mag_force(3,:) = mag_force(3,:) + dHf(3)
end subroutine ext_mag_field
